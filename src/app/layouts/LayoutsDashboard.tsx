...
import { ApiError, authApi, layoutsApi, type AuthUser, type LayoutMeta } from '@/lib/api-client';
import { LAYOUT_SCHEMA_VERSION, LayoutPayloadSchema, migrateLayoutPayload, type LayoutPayload } from '@/lib/schemas/layout';
...
export function LayoutsDashboard({
  ...
  const refresh = useCallback(async () => {
    setLayouts(await layoutsApi.list());
  }, []);
  ...
  async function onUploadJson(file: File) {
    ...
    try {
      ...
      setLayouts(prev => [{ ...layout, snapshotCount: 1, latestSnapshotAt: layout.updatedAt }, ...prev]);
